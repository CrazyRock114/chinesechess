...
// ============================================================
import { ROWS, COLS, RED, BLACK, getMoves, legalMoves, kingsFacing, kingPos, inCheck, hashBoard } from './game.js?v=40a83e61b5';
...
export function findBestMove(srcBoard, side, level = 'medium', recent = []) {
  ...
  // 開局庫匹配：開局階段若命中大師定式，直接秒出著法
  if (pieceCount >= 26) {
    const bookMove = getOpeningMove(b, side, level);
    if (bookMove) {
      const isLegal = rootMoves.some(
        (m) => m.fr === bookMove.from.r && m.fc === bookMove.from.c &&
               m.tr === bookMove.to.r && m.tc === bookMove.to.c
      );
      if (isLegal) {
        return { from: bookMove.from, to: bookMove.to, score: 0, depth: 0, isBook: true };
      }
    }
  }
  ...
  if (completed >= 1 && scored[0].score <= MATE - 200 && cfg.window > 0) {
    ...
    let top = near.filter((e) => best - e.score <= cfg.window);
    // 若最佳著法為吃子，隨機候選優先限於吃子著法，避免中等難度隨機放棄白吃大子
    if (b[near[0].m.tr][near[0].m.tc]) {
      const capTop = top.filter((e) => b[e.m.tr][e.m.tc]);
      if (capTop.length) top = capTop;
    }
